...
'''
''' 
def analyse(tweetlist):
'''
'''

	sentencelist =reduce(lambda x,y: x.append(y), map(nltk.sent_tokenize, tweetlist))
	hatekeywordlist=None
	lovekeywordlist=None

	for sentence in sentencelist:
	
		atweet=analyse_sentence(sentence)
		#if(sentence==0)
		#	continue
		if(atweet==1)
			lovekeywordlist.append(analyse_sentence(sentence))
		else:
			hatekeywordlist.append(sentence)

	return (hatekeywordlist,lovekeywordlist)
